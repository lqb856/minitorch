...
__global__ void reduced_sum_axis_zero(const float *input_data,
                                      float *output_data, int input_n,
                                      int output_n) {
...
int DLGpuReduceSumAxisZero(const DLArrayHandle input, DLArrayHandle output) {
  ...
  reduced_sum_axis_zero<<<n_blocks, thread_per_block>>>(input_data, output_data,
                                                        input_n, output_n);
  return 0;
}
...
__global__ void matrix_elementwise_subtract(const float *a, const float *b,
                                            float *c, int n) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < n) {
    c[index] = a[index] - b[index];
...
int DLGpuMatrixElementwiseSubtract(const DLArrayHandle matA,
  ...
  matrix_elementwise_subtract<<<num_blocks, threads_per_block>>>(
      data_A, data_B, data_output, n);
  return 0;
}

__global__ void matrix_elementwise_division(const float *a, const float *b,
                                            float *result, int n) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < n) {
...
int DLGpuMatrixElementwiseDiv(const DLArrayHandle matA,
  ...
  matrix_elementwise_division<<<num_blocks, threads_per_block>>>(
      data_A, data_B, data_output, n);
  return 0;
}

__global__ void matrix_elementwise_add_by_const_kernal(const float *d_in,
                                                       float *d_out, float val,
                                                       int n) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < n) {
...
int DLGpuMatrixElementwiseSubtractByConst(const DLArrayHandle input, float val,
  ...
  int threads_per_block = 1024;
  int num_blocks = (n + threads_per_block - 1) / threads_per_block;
  matrix_elementwise_subtract_by_const_kernal<<<num_blocks,
                                                threads_per_block>>>(
      input_data, output_data, val, n);
  return 0;
}
...
int DLGpuMatrixElementwiseMultiply(const DLArrayHandle matA,
  ...
  elementwise_mul_kernel<<<num_blocks, threads_per_block>>>(
      mat_a_data, mat_b_data, output_data, n);

  return 0;
}

__global__ void matrix_elementwise_sqrt(const float *d_input, float *d_output,
                                        int n) {
  int index = blockDim.x * blockIdx.x + threadIdx.x;
  if (index < n) {
...
int DLGpuMatrixElementwiseSqrt(const DLArrayHandle input,
                               DLArrayHandle output) {
  /* TODO: Your code here */
  int n = 1;
  for (int i = 0; i < input->ndim; i++) {
  ...
  int threads_per_block = 1024;
  int num_blocks = (n + threads_per_block - 1) / threads_per_block;
  matrix_elementwise_sqrt<<<num_blocks, threads_per_block>>>(input_data,
                                                             output_data, n);
  return 0;
}
...
int DLGpuMatrixMultiplyByConst(const DLArrayHandle input, float val,
  ...
  int threads_per_block = 1024;
  int num_blocks = (n + threads_per_block - 1) / threads_per_block;
  marix_multiply_by_const<<<num_blocks, threads_per_block>>>(
      input_data, output_data, val, n);
  return 0;
}
...
int DLGpuMatrixMultiply(const DLArrayHandle matA, bool transposeA,
  ...
  cublasSgemm(cublas_handle, transposeB ? CUBLAS_OP_T : CUBLAS_OP_N,
...
int DLGpuReluGradient(const DLArrayHandle input, const DLArrayHandle in_grad,
  ...
  relu_gradient_kernel<<<num_blocks, threads_per_block>>>(
      input_data, output_data, in_grad_data, n);
  return 0;
}
